# OnOffTempLan - Ethernet UDP sensor transmitter

This project keeps the original OnOffTemp behavior:

- DHT temperature and humidity measurement
- switch-controlled system ON/OFF state
- green/red LED status indication

It adds a LAN-based UDP sender so the measured temperature and humidity can be sent over Ethernet to a local router, ideally using a physical LAN cable.

## Hardware

- MCU: STM32 NUCLEO-H563ZI
- Sensor: DHT22 (or DHT11) temperature/humidity
- Switch: D2
...
- Board IP: 192.168.0.51
- Router LAN IP: 192.168.0.1
- UDP destination port: 5006
...
- DHT sensors require at least 2 seconds between reads, so data is sent every 2 seconds when ON.
